        ...
        if(message != null) {
            ...
            if(message.withDrawStatus) {
                ...
                if("Hangup".equals(NullUtil.checkNull(message.videoStatus)) || "hangup".equals(NullUtil.checkNull(message.videoStatus))) {
                    ...
                    holder.getDescTextView().setText("通话时长: "+ time);
                    holder.getDescTextView().setTextColor(context.getResources().getColor(R.color.all_black));
                    holder.getVideoIcon().setImageResource(R.drawable.kf_chatrow_video);

                }else if("cancel".equals(NullUtil.checkNull(message.videoStatus))) {
                    //取消
                    holder.getDescTextView().setText("视频会话已取消");
                    holder.getDescTextView().setTextColor(context.getResources().getColor(R.color.all_black));
                    holder.getVideoIcon().setImageResource(R.drawable.kf_chatrow_video);

                }else if("refuse".equals(NullUtil.checkNull(message.videoStatus))) {
                    //拒绝
                    holder.getDescTextView().setText("拒绝了视频邀请");
                    holder.getDescTextView().setTextColor(context.getResources().getColor(R.color.all_black));
                    holder.getVideoIcon().setImageResource(R.drawable.kf_chatrow_video);
                }
